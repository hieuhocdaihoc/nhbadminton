...
const SingleBookings = () => {
    ...
    const statusConfig = {
        ...
        confirmed: { dot: 'bg-blue-500', text: 'text-blue-700', bg: 'bg-blue-50', label: 'Đã xác nhận' },
    ...
    const inputClass = "w-full bg-[#f8f8fa] border border-zinc-200 rounded-lg px-3.5 py-2.5 text-sm text-zinc-800 outline-none focus:border-zinc-400 focus:ring-1 focus:ring-zinc-200 transition-all placeholder:text-zinc-400";
        ...
        <div className="max-w-[1400px] mx-auto space-y-5">
            ...
            <div className="bg-white rounded-xl border border-zinc-200/60 p-4">
                <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                    ...
                    <div className="relative w-full lg:w-80">
                        ...
                        <input type="text" placeholder="Tìm tên, SĐT hoặc mã đơn..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-xl border border-zinc-200/60 p-16 text-center">
                    ...
                    <p className="text-sm text-zinc-400">Không tìm thấy đơn đặt sân nào</p>
                </div>
            ) : (
                        ...
                        <table className="w-full min-w-[900px]">
                            <thead>
                                <tr className="text-[10px] font-medium text-zinc-400 uppercase tracking-wider bg-zinc-50/60 border-b border-zinc-100">
                                    <th className="text-left py-3 px-5" style={{width:'260px'}}>Khách hàng</th>
                                    <th className="text-left py-3 px-3" style={{width:'110px'}}>Ngày</th>
                                    <th className="text-left py-3 px-3" style={{width:'110px'}}>Giờ</th>
                                    <th className="text-left py-3 px-3" style={{width:'70px'}}>Sân</th>
                            ...
                            <tbody>
                                {filteredBookings.map((b) => {
                                    ...
                                    const dateStr = detail?.booking_date ? new Date(detail.booking_date).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '—';
                                        ...
                                        <tr key={b.id} className={`border-b border-zinc-100 last:border-b-0 hover:bg-zinc-50/40 transition-colors group ${isCancelled ? 'opacity-45' : ''}`}>
                                            <td className="py-3.5 px-5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[11px] text-zinc-400 font-mono truncate">{b.booking_code} · {b.customer_phone}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-3.5 px-3 text-xs text-zinc-600">{dateStr}</td>
                                            ...
                                            <td className="py-3.5 px-5 text-right">
                                                <div className="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                                    {b.status === 'pending' && (
                                                        <button onClick={() => requestAction(b, 'confirm')} className="px-2.5 py-1 bg-zinc-900 text-white rounded text-[10px] font-medium hover:bg-zinc-800 transition-colors">Duyệt</button>
                                                    )}
                                                    {b.payment_status !== 'paid' && !isCancelled && (
                                                        <button onClick={() => requestAction(b, 'pay')} className="px-2.5 py-1 bg-lime-600 text-white rounded text-[10px] font-medium hover:bg-lime-700 transition-colors">Thu tiền</button>
                                                    )}
                                                    {!isCancelled && b.status !== 'completed' && (
                                                        <button onClick={() => openRescheduleModal(b)} className="px-2.5 py-1 border border-zinc-200 text-zinc-500 rounded text-[10px] hover:bg-zinc-50 transition-colors">Đổi lịch</button>
                                                    )}
                                                    {!isCancelled && b.status !== 'completed' && (
                                                        <button onClick={() => requestAction(b, 'cancel')} className="px-2 py-1 text-zinc-400 rounded text-[10px] hover:text-red-500 hover:bg-red-50 transition-colors">Hủy</button>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>
            ...
            {pagination.last_page > 1 && (
                <div className="flex justify-between items-center py-2">
                    <span className="text-xs text-zinc-400">Trang {pagination.current_page} / {pagination.last_page}</span>
                    <div className="flex gap-2">
                        <button disabled={pagination.current_page === 1} onClick={() => fetchData(pagination.current_page - 1)}
                            className="px-3.5 py-2 border border-zinc-200 rounded-lg text-xs text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors">← Trước</button>
                        <button disabled={pagination.current_page === pagination.last_page} onClick={() => fetchData(pagination.current_page + 1)}
                            className="px-3.5 py-2 border border-zinc-200 rounded-lg text-xs text-zinc-600 hover:bg-zinc-50 disabled:opacity-40 transition-colors">Tiếp →</button>
                    </div>
                </div>
            )}
                    ...
                    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="px-6 py-5 border-b border-zinc-100">
                                ...
                                <p className="text-xs text-zinc-400 mt-0.5">Khách: <span className="text-zinc-600">{rescheduleModal.booking?.customer_name}</span></p>
                            </div>
                ...
                {confirmModal.isOpen && (
                    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setConfirmModal({ isOpen: false, title: '', message: '', actionData: null })} className="flex-1 py-2.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-lg text-xs font-medium transition-colors">Hủy</button>
