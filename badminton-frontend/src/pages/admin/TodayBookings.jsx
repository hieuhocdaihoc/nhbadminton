...
const TodayBookings = () => {
    ...
    const handleAddItemSubmit = async (e) => {
        e.preventDefault();
        if (!itemForm.selected_val) return alert("Vui lòng chọn một mặt hàng!");
        const [type, item_id] = itemForm.selected_val.split('|');
        setIsAddingItem(true);
        try {
            await adminBookingService.addItemToBooking(addItemModal.booking.id, {
                type, item_id, quantity: Number(itemForm.quantity), note: itemForm.note
            });
            
            // Nếu khách đã thanh toán tiền sân trước đó (thường là thanh toán online),
            // nhưng giờ phát sinh thêm tiền nước/dịch vụ -> Đổi trạng thái thành partially_paid (Nợ Pro-shop)
            ...
            setMessage({ type: 'success', text: 'Thêm món thành công!' });
    ...
    const handleOpenBill = (booking) => { setSelectedBill(booking); setIsBillModalOpen(true); };
        ...
        <div className="max-w-[1400px] mx-auto space-y-5">
            ...
            <div className="bg-white rounded-xl border border-zinc-200/60 p-4">
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                    <div className="relative w-full sm:w-72">
                        ...
                        <input type="text" placeholder="Tìm tên hoặc SĐT..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                    ...
                    <div className="flex items-center gap-1 bg-zinc-100 p-1 rounded-lg">
                        {filterTabs.map(tab => (
                            <button key={tab.key} onClick={() => setFilterStatus(tab.key)}
                                ...
                                {tab.label}
                                {tab.count > 0 && <span className={`ml-1.5 text-[9px] px-1.5 py-0.5 rounded-full ${filterStatus === tab.key ? 'bg-zinc-900 text-white' : 'bg-zinc-200 text-zinc-500'}`}>{tab.count}</span>}
                            </button>
                        ))}
                    </div>
                </div>
                        ...
                        <table className="w-full min-w-[1000px]">
                            ...
                            <tbody>
                                {filteredBookings.map(b => {
                                    ...
                                    return (
                                        <tr key={b.id} className={`border-b border-zinc-100 last:border-b-0 hover:bg-zinc-50/40 transition-colors group ${isCancelled ? 'opacity-45' : ''}`}>
                                            <td className="py-3.5 px-5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-[13px] font-medium text-zinc-800 truncate">{b.customer_name}</p>
                                                        <p className="text-[11px] text-zinc-400 font-mono truncate">{b.customer_phone}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-3.5 px-3">
                                                <span className="text-xs font-mono text-zinc-600">{timeStr}</span>
                                            </td>
                                            <td className="py-3.5 px-3 text-xs text-zinc-600">{courtName}</td>
                                            <td className="py-3.5 px-3">
                                                {!isCancelled && (
                                                    <button onClick={() => openAddItemModal(b)} className="px-2.5 py-1 text-zinc-500 border border-dashed border-zinc-300 rounded text-[10px] hover:bg-zinc-50 hover:border-zinc-400 transition-colors">
                                                        + Thêm món
                                                    </button>
                                                )}
                                            </td>
                                            <td className="py-3.5 px-3 text-right">
                                                <p className="text-sm font-semibold text-zinc-800">{Number(b.total_price).toLocaleString()}₫</p>
                                                {b.payment_status === 'paid' ? (
                                                    <p className="text-[10px] text-emerald-600">✓ Đã thu đủ</p>
                                                ) : b.payment_status === 'partially_paid' ? (
                                                    <p className="text-[10px] font-bold text-amber-600">⚠ Nợ Pro-shop</p>
                                                ) : (
                                            ...
                                            <td className="py-3.5 px-5 text-right">
                                                <div className="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => handleOpenBill(b)} className="px-2.5 py-1 border border-zinc-200 text-zinc-500 rounded text-[10px] hover:bg-zinc-50 transition-colors" title="Xem Bill">🧾 Bill</button>
                                                    {b.status === 'pending' && (
                                                        <button onClick={() => handleAction(b.id, 'confirmed', b.payment_status)} className="px-2.5 py-1 bg-zinc-900 text-white rounded text-[10px] font-medium hover:bg-zinc-800 transition-colors">Duyệt</button>
                                                    )}
                                                    {b.payment_status !== 'paid' && !isCancelled && (
                                                        <button onClick={() => handleAction(b.id, b.status === 'pending' ? 'confirmed' : b.status, 'paid')} className="px-2.5 py-1 bg-lime-600 text-white rounded text-[10px] font-medium hover:bg-lime-700 transition-colors">
                                                            {b.payment_status === 'partially_paid' ? 'Thu nợ DV' : 'Thu tiền'}
                                                        </button>
                                                    )}
                                                </div>
                                            </td>
                    ...
                    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            ...
                            <div className="px-6 py-5 border-b border-zinc-100">
                                <h3 className="text-sm font-semibold text-zinc-800">Bán thêm dịch vụ</h3>
                                <p className="text-xs text-zinc-400 mt-0.5">Khách: <span className="text-zinc-600">{addItemModal.booking?.customer_name}</span></p>
                            </div>
                            <form onSubmit={handleAddItemSubmit} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-[11px] font-medium text-zinc-500 mb-1.5">Chọn mặt hàng / dịch vụ</label>
                                    <select required value={itemForm.selected_val} onChange={(e) => setItemForm({ ...itemForm, selected_val: e.target.value })} className={inputClass}>
                                        <option value="" disabled>— Vui lòng chọn —</option>
                                        {products.length > 0 && (
                                            <optgroup label="🥤 Sản phẩm">
                                                {products.map(p => (
                                                    <option key={`p_${p.id}`} value={`product|${p.id}`} disabled={p.stock_quantity <= 0}>
                                        ...
                                        {services.length > 0 && (
                                            <optgroup label="🏸 Dịch vụ">
                                                {services.map(s => (
                                                    <option key={`s_${s.id}`} value={`service|${s.id}`} disabled={s.status === 'inactive'}>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="number" min="1" required value={itemForm.quantity} onChange={(e) => setItemForm({ ...itemForm, quantity: e.target.value })} className={inputClass} />
                                    </div>
                                    <div>
                                        <label className="block text-[11px] font-medium text-zinc-500 mb-1.5">Ghi chú</label>
                                        <input type="text" placeholder="Tùy chọn..." value={itemForm.note} onChange={(e) => setItemForm({ ...itemForm, note: e.target.value })} className={inputClass} />
                                    </div>
                                </div>
                                <div className="flex gap-2 pt-3 border-t border-zinc-100">
                                    <button type="button" onClick={() => setAddItemModal({ isOpen: false, booking: null })} className="flex-1 py-2.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-lg text-xs font-medium transition-colors">Hủy</button>
                                    <button type="submit" disabled={isAddingItem} className="flex-1 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg text-xs font-medium transition-colors">
                                        {isAddingItem ? 'Đang thêm...' : 'Thêm vào Bill'}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
            ...
            <AnimatePresence>
                {isBillModalOpen && selectedBill && (
                    <div onClick={() => setIsBillModalOpen(false)} className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                            onClick={e => e.stopPropagation()}
                            className="bg-white w-full max-w-[340px] shadow-xl rounded-2xl p-6 relative font-mono text-zinc-900">

                            <div className="text-center mb-5">
                                <h2 className="text-lg font-black tracking-tight uppercase leading-none mb-1">NH BADMINTON</h2>
                                <p className="text-[10px] text-zinc-400">Sân cầu lông · SĐT: 0987.654.321</p>
                                <div className="mt-4 pt-3 border-t border-dashed border-zinc-300">
                                    <p className="text-xs font-semibold uppercase text-zinc-600">Hóa đơn thanh toán</p>
                                </div>
                            </div>

                            <div className="space-y-1.5 text-xs mb-4">
                                <div className="flex justify-between"><span className="text-zinc-500">Mã GD:</span> <strong>#{selectedBill.booking_code?.split('_')[1] || selectedBill.id?.slice(0, 6)}</strong></div>
                                <div className="flex justify-between"><span className="text-zinc-500">Ngày:</span> <span>{new Date().toLocaleString('vi-VN')}</span></div>
                                <div className="flex justify-between"><span className="text-zinc-500">Khách:</span> <strong className="truncate max-w-[140px] text-right">{selectedBill.customer_name}</strong></div>
                            </div>
                            ...
                            <table className="w-full text-[11px] mb-4 text-left">
                                ...
                                <tbody className="border-b border-dashed border-zinc-300">
                                    <tr>
                                        ...
                                        <td className="py-2 text-right align-top font-semibold text-xs">{Number(selectedBill.subtotal_court || selectedBill.total_price).toLocaleString()}</td>
                                    </tr>
                                    {selectedBill.service_details?.map((item) => (
                                        <tr key={item.id} className="border-t border-dotted border-zinc-200">
                                            <td className="py-2 pr-2">
                                                <strong className="block text-[11px]">{item.product?.name || item.service?.name || 'Dịch vụ'}</strong>
                                                {item.note && <span className="text-[9px] text-zinc-400 block">*{item.note}</span>}
                                            </td>
                                            <td className="py-2 text-center align-top text-zinc-600">{item.quantity}</td>
                                            <td className="py-2 text-right align-top font-semibold">{Number(item.total_price).toLocaleString()}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            ...
                            <div className="space-y-1 text-xs mb-5">
                                <div className="flex justify-between text-zinc-500"><span>Tiền sân:</span> <span>{Number(selectedBill.subtotal_court || selectedBill.total_price).toLocaleString()}đ</span></div>
                                <div className="flex justify-between text-zinc-500"><span>Dịch vụ thêm:</span> <span>{Number(selectedBill.subtotal_service || 0).toLocaleString()}đ</span></div>
                            ...
                            <p className="text-center text-[10px] text-zinc-400 mb-6">
                                Cảm ơn quý khách!<br />Wifi: NH_Badminton · Pass: 88888888
                            </p>

                            <div className="flex gap-2 font-sans print:hidden">
                                <button onClick={() => setIsBillModalOpen(false)} className="flex-1 py-2.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-lg text-xs font-medium transition-colors">Đóng</button>
                                <button onClick={() => alert('Kết nối máy in...')} className="flex-1 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg text-xs font-medium transition-colors">🖨️ In Bill</button>
                            </div>
                        </motion.div>
                    </div>
                )}
