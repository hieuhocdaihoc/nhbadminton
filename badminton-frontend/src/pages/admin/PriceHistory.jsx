...
const PriceHistory = ({ embedded = false }) => {
  ...
  const fetchHistory = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await pricingService.getPriceHistory();
  ...
  return (
    <div className={embedded ? "space-y-5" : "admin-page-container"}>
      <div className="admin-page-header">
        {!embedded && (
          <div>
            <h1 className="admin-page-title">Lịch sử sửa giá</h1>
            <p className="admin-page-subtitle">Biến động giá, người thực hiện và thời điểm thay đổi.</p>
          </div>
        )}
      </div>
      ...
      <div className="admin-stat-group flex-wrap">
        ...
        {summary.map((s, i) => (
          <div key={i} className="admin-stat-badge badge-default px-4 py-2">
            <div className="admin-stat-value val-default">{s.change_count}</div>
            <div className="admin-stat-label lbl-default">{s.label ?? s.court_name}</div>
          </div>
        ))}
      </div>
                ...
                return (
                  <tr key={h.id}>
                    ...
                    <td className="text-right text-sm text-zinc-500">
                      {h.old_price !== null ? `${fmt(h.old_price)}đ` : "—"}
                    </td>
                    <td className="text-right text-sm font-semibold">
                      {h.new_price !== null ? `${fmt(h.new_price)}đ` : "—"}
                    </td>
              ...
              {histories.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-14 text-center text-sm text-zinc-400">
                    Chưa có lịch sử sửa giá nào. Lịch sử sẽ được ghi lại từ lần sửa giá tiếp theo.
                  </td>
                </tr>
              )}
            </tbody>
