...
const INITIAL_BOOKINGS = {
    '15:00-1': { id: 'B1', user: 'Minh Tuấn', phone: '0912345888', type: 'Cọc online', status: 'confirmed', price: 100000, paid: true, note: 'Khách quen hay uống Revive' },
    '16:00-1': { id: 'B2', user: 'Minh Tuấn', phone: '0912345888', type: 'Cọc online', status: 'confirmed', price: 100000, paid: true, note: '' },
    '18:00-2': { id: 'B3', user: 'Hoàng Long', phone: '0903555123', type: 'Khách vãng lai', status: 'pending', price: 120000, paid: false, note: 'Giữ chỗ qua điện thoại' },
    '19:00-2': { id: 'B4', user: 'CLB Smashers', phone: '0888999777', type: 'Cố định tháng', status: 'checked-in', price: 120000, paid: true, note: 'Nhóm 8 người' },
    '20:00-2': { id: 'B5', user: 'CLB Smashers', phone: '0888999777', type: 'Cố định tháng', status: 'checked-in', price: 120000, paid: true, note: 'Nhóm 8 người' },
    '17:00-3': { id: 'B6', user: 'Thanh Hương', phone: '0933444556', type: 'Cọc online', status: 'confirmed', price: 120000, paid: true, note: '' },
};
...
const BookingManager = () => {
    ...
    const [quickSlot, setQuickSlot] = useState({ courtId: 1, courtName: '', time: '' });
    ...
    const handleDeleteBooking = () => {
        ...
        if (window.confirm(`⚠️ Xác nhận hủy lịch của "${current.user}" lúc ${activeBookingKey.split('-')[0]}?`)) {
            const copy = { ...bookings };
    ...
    // ========================================================
    const stats = Object.values(bookings).reduce((acc, curr) => {
        acc.totalSlots += 1;
        ...
        return acc;
    }, { totalSlots: 0, revenue: 0, collected: 0, pendingCount: 0, activePlayers: 0 });
        ...
        <div className="space-y-6 max-w-[1600px] mx-auto pb-12">
            ...
            <div className="bg-white rounded-2xl border border-zinc-100 overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-zinc-100 flex flex-wrap justify-between items-center bg-zinc-50/50 gap-2">
                    <span className="text-xs font-black text-zinc-800 uppercase tracking-wider">Sa Bàn Giờ Trực Tuyến</span>
                    <div className="flex items-center gap-4 text-xs font-medium text-zinc-600">
                        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-zinc-50 border border-zinc-200 block"></span> Trống</span>
                        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-50 border border-amber-300 block"></span> Chưa cọc</span>
                        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-blue-50 border border-blue-300 block"></span> Đã cọc</span>
                        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-50 border border-emerald-300 block"></span> Đang chơi</span>
                    </div>
                </div>
                    ...
                    <table className="w-full border-collapse text-left min-w-[1100px]">
                        <thead>
                            <tr className="bg-zinc-100 text-zinc-700 text-xs font-bold uppercase tracking-wider border-b border-zinc-200">
                                <th className="p-4 w-28 text-center border-r border-zinc-200 sticky left-0 bg-zinc-100/95 backdrop-blur-xs z-10">Khung giờ</th>
                                {MOCK_COURTS.map(court => (
                                    <th key={court.id} className="p-4 text-center border-r border-zinc-200 min-w-[200px] w-[20%]">
                                        <div className="font-black text-sm text-zinc-900">{court.name}</div>
                                        {court.id === 5 ? (
                                            <span className="text-[9px] font-bold text-red-600 block mt-0.5">Dừng phục vụ</span>
                                        ) : (
                                            <span className="text-[9px] font-bold text-emerald-600 block mt-0.5">● Khả dụng</span>
                                        )}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody className="divide-y divide-zinc-200 text-sm">
                            {hours.map(hour => {
                                ...
                                return (
                                    <tr key={hour} className="hover:bg-zinc-50/60 transition-colors">

                                        {/* Cột Khung giờ */}
                                        <td className={`p-4 text-center border-r border-zinc-200 sticky left-0 backdrop-blur-xs shadow-2xs z-10 ${isPrimeTime ? 'bg-amber-50/90 text-amber-950' : 'bg-white/95 text-zinc-900'}`}>
                                            <span className="text-base font-black block tracking-tight">{hour}</span>
                                            <span className={`text-[9px] font-bold block mt-0.5 ${isPrimeTime ? 'text-amber-600' : 'text-zinc-400'}`}>
                                        ...
                                        {MOCK_COURTS.map(court => {
                                            ...
                                            if (court.id === 5) {
                                                return (
                                                    <td key={slotKey} className="p-3 border-r border-zinc-200 bg-zinc-100/70 text-center align-middle">
                                                        <span className="text-xs text-zinc-400 font-bold block">🔒 Bảo trì</span>
                                                    </td>
                                                );
                                            }
                                            ...
                                            if (booking) {
                                                ...
                                                if (filterStatus !== 'all' && booking.status !== filterStatus) {
                                                    return <td key={slotKey} className="p-2 border-r border-zinc-200 bg-zinc-50/20"></td>;
                                                }
                                                ...
                                                return (
                                                    <td key={slotKey} className="p-2 border-r border-zinc-200 align-top">
                                                        <div
                                                            onClick={() => handleOpenActionModal(slotKey)}
                                                            className={`p-3 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between h-full min-h-[120px] shadow-2xs hover:shadow-md relative group ${isPending ? 'bg-amber-50/90 border-amber-300 hover:border-amber-500' :
                                                                isConfirmed ? 'bg-blue-50/90 border-blue-300 hover:border-blue-500' :
                                                        ...
                                                        >
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="flex items-start justify-between gap-1 mb-1">
                                                                    ...
                                                                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase shrink-0 ${booking.paid ? 'bg-emerald-200/80 text-emerald-800 font-black' : 'bg-amber-200/80 text-amber-800'
                                                                        }`}>
                                                                        {booking.paid ? 'Đã thanh toán' : 'Chưa thu'}
                                                                    </span>
                                                                </div>

                                                                <p className="text-[11px] font-mono text-zinc-500 mt-0.5">📞 {booking.phone}</p>
                                                            ...
                                                            <div className="mt-3 pt-2 border-t border-black/5 flex items-center justify-between text-[10px] font-bold">
                                                                <span className="text-zinc-500">{booking.price.toLocaleString()} đ</span>
                                                                <span className={isPending ? 'text-amber-700' : isConfirmed ? 'text-blue-700' : 'text-emerald-700'}>
                                                                    {isPending ? '⏳ Chờ cọc' : isConfirmed ? '✓ Đã giữ' : '🏸 Đang thi đấu'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </td>
                                            ...
                                            return (
                                                <td key={slotKey} className="p-2 border-r border-zinc-200 align-middle text-center group">
                                                    <div
                ...
                <div className="p-4 bg-zinc-50 border-t border-zinc-100 text-center text-xs text-zinc-500 font-medium">
                    💡 <strong>Quy tắc hệ thống:</strong> Giờ vàng áp dụng tự động giá cao điểm. Lịch đặt chưa cọc quá 20 phút trước ca sẽ bị cảnh báo để ưu tiên cho khách vãng lai.
                </div>
            </div>
                    ...
                    >
                        ...
                        <div className="p-6 bg-zinc-50 border-b border-zinc-100 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-zinc-500 font-medium mt-0.5">
                                    Khung: <strong className="text-blue-600">{activeBookingKey.split('-')[0]}</strong> ● Sân số {activeBookingKey.split('-')[1]}
                                </p>
                            </div>
                            <button onClick={() => setActionModalOpen(false)} className="text-zinc-400 hover:text-zinc-600 font-bold">✕</button>
                        </div>
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-200/60 flex items-center justify-between">
                                <div>
                                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Chi phí sân</p>
                                    <p className="text-xl font-black text-zinc-900 mt-0.5">{activeBooking.price.toLocaleString()} VNĐ</p>
                                    <p className="text-[10px] text-zinc-400 font-medium">Hình thức: {activeBooking.type}</p>
                                </div>

                                <div className="text-right">
                                    <span className={`inline-block px-3 py-1 rounded-xl text-xs font-black uppercase mb-2 ${activeBooking.paid ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-amber-100 text-amber-800 border border-amber-200'
                                        }`}>
                                    ...
                                    >
                                        🔄 {activeBooking.paid ? 'Chuyển về chưa thu' : 'Xác nhận đã thu tiền'}
                                    </button>
                                </div>
                            </div>
                ...
                >
                    ...
                    >
                        <div className="p-6 bg-blue-600 text-white flex items-center justify-between">
                            ...
                            <button onClick={() => setQuickModalOpen(false)} className="text-blue-200 hover:text-white font-bold">✕</button>
                        </div>

                        <form onSubmit={handleSubmitQuickBooking} className="p-6 space-y-4">
                            <div>
                                <label className="block text-[11px] font-bold text-zinc-500 uppercase mb-1">Tên khách hàng *</label>
                                <input
                                    type="text"
                                    required
                                    autoFocus
                                    ...
                                    onChange={e => setQuickForm({ ...quickForm, user: e.target.value })}
                            ...
                            <div>
                                <label className="block text-[11px] font-bold text-zinc-500 uppercase mb-1">Số điện thoại</label>
                                <input
                                    type="tel"
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-[11px] font-bold text-zinc-500 uppercase mb-1">Phân loại</label>
                                    <select
                                ...
                                <div>
                                    <label className="block text-[11px] font-bold text-zinc-500 uppercase mb-1">Chi phí (VNĐ)</label>
                                    <input
                                        type="number"
                            ...
                            <div>
                                <label className="block text-[11px] font-bold text-zinc-500 uppercase mb-1">Ghi chú</label>
                                <input
                                    type="text"
                            ...
                            <div className="pt-2 border-t border-zinc-100 flex items-center justify-between">
                                <span className="text-xs font-bold text-zinc-700">Trạng thái thanh toán</span>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={quickForm.paid}
                                        onChange={e => setQuickForm({ ...quickForm, paid: e.target.checked })}
                                        className="w-4 h-4 text-blue-600 rounded"
                                    />
                                    <span className="text-xs font-bold text-emerald-600">Thu tiền ngay</span>
                                </label>
                            </div>
