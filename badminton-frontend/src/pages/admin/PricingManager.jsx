...
const PricingManager = () => {
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      if (modalMode === "create") {
        ...
        setMessage({
          type: "success",
          text: `Đã cấu hình giá cho "${selectedCourt.name}"!`,
        });
      } else {
  ...
  const currentCourtPricings = selectedCourt
    ? pricings.filter(
        (p) => p.court_id.toString() === selectedCourt.id.toString(),
      )
    : [];
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    <td
                      colSpan="5"
                      className="py-16 text-center text-xs text-zinc-300"
                    >
                      ← Chọn sân bên trái
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    >
                      Chưa cấu hình giá cho sân này
                    </td>
                  </tr>
                ) : (
                  currentCourtPricings.map((item) => {
                    const dtc =
                      dayTypeConfig[item.day_type] || dayTypeConfig.weekday;
