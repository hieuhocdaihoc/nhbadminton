...
const Categories = () => {
  ...
  const handleDeleteClick = async (category) => {
    if (
      window.confirm(
        `Bạn có chắc chắn muốn tạm ẩn danh mục [${category.name}]?`,
      )
    ) {
      try {
            ...
            <table className="w-full text-left">
              <thead className="bg-zinc-50/60 border-b border-zinc-100 text-[10px] font-medium text-zinc-400 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="py-3 px-3" style={{ width: "300px" }}>
                    Mô tả
                  </th>
                  <th
              ...
              <tbody>
                {isLoading ? (
                  ...
                  categories.map((c) => (
                    ...
                    >
                      <td className="py-3.5 px-5 text-sm font-semibold text-zinc-800">
                        {c.name}
                      </td>
                      ...
                      <td className="py-3.5 px-3 text-center">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium ${c.status === "active" ? "text-emerald-700 bg-emerald-50" : "text-zinc-500 bg-zinc-100"}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${c.status === "active" ? "bg-emerald-500" : "bg-zinc-400"}`}
                          />
