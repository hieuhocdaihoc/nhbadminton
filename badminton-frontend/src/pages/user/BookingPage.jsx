...
const BookingPage = () => {
    ...
    const handleFinalizeBooking = async (e) => {
        ...
        if (selectedSlots.length === 0) {
            return alert('⚠️ Vui lòng chọn ít nhất một khung giờ trống!');
        }
        ...
        try {
            ...
            if (response.status === 201 || response.data?.status === 'success') {
                ...
                const bookingId = responseData?.booking_id || responseData?.payment_booking_id;

                setCreatedBookingInfo(responseData);

                if (isPrepaid) {
                    if (!bookingId) {
                        alert('Đơn đã tạo nhưng chưa lấy được mã thanh toán. Vui lòng kiểm tra backend trả booking_id.');
                        return;
                    }
                ...
                if (isLoggedIn()) {
                    ...
                    setSuccessModal({
                        ...
                        message: 'Vui lòng lưu lại mã đơn để tra cứu lịch đặt sân sau này.',
    ...
    const handleClosePaymentModal = () => {
        ...
        if (isLoggedIn()) {
            ...
            setSuccessModal({
                ...
                message: 'Nếu bạn đã chuyển khoản, hệ thống sẽ xác nhận sau khi nhận giao dịch. Vui lòng lưu mã đơn để tra cứu.',
                bookings: createdBookingInfo.bookings
            });
        }
    };
                                        ...
                                        >
                                            <div className="flex items-center gap-2 mb-4">
                                                ...
                                                <span className="text-[10px] text-zinc-600 font-semibold ml-auto">
                                                    {session.slots.filter(s => s.is_available).length} trống
                                                </span>
                                            </div>
                                ...
                                {bookingType === 'recurring' && (
                                    ...
                                    >
                                        ...
                                        <p className="text-[9px] text-purple-400/70 italic">
                                            * Tự động lặp khung giờ đã chọn cho các tuần tiếp theo.
                                        </p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <form onSubmit={handleFinalizeBooking} className="space-y-4">
                                ...
                                {bookingType === 'single' ? (
                                    ...
                                    <div className="p-3 bg-purple-500/10 text-purple-400 text-[10px] font-bold text-center rounded-xl uppercase border border-purple-500/20 tracking-wider">
                                        Đặt sân định kỳ bắt buộc thanh toán online trước
                                    </div>
                                )}
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex justify-between gap-4">
                                        ...
                                        <span className="font-extrabold text-lime-600 text-right">
                                            {Number(paymentInfo.remaining_amount || 0).toLocaleString('vi-VN')} VNĐ
                                        </span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {successModal.bookings?.map((booking, index) => (
                                            ...
                                            >
                                                ...
                                                <p className="text-[11px] text-zinc-500 mt-1">
                                                    Số tiền: {Number(booking.total_price || 0).toLocaleString('vi-VN')}đ
                                                </p>
                                            </div>
                                        ))}
                                    </div>
